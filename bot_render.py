import logging
import os
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, ContextTypes
import random
from keep_alive import keep_alive

# Запускаем keep-alive сервер для Render
keep_alive()

# Настройка логирования
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.INFO
)
logger = logging.getLogger(__name__)

# База вопросов для разных уровней
QUESTIONS = {
    'junior': [
        {
            'question': 'Что такое тестирование ПО?',
            'options': [
                'Процесс выявления дефектов в программном обеспечении',
                'Процесс написания кода',
                'Процесс установки программ',
                'Процесс удаления багов'
            ],
            'correct': 0,
            'explanation': 'Тестирование ПО - это процесс проверки соответствия между реальным и ожидаемым поведением программы.'
        },
        {
            'question': 'Что такое баг (дефект)?',
            'options': [
                'Отклонение фактического результата от ожидаемого',
                'Новая функциональность',
                'Документация к продукту',
                'Тестовый сценарий'
            ],
            'correct': 0,
            'explanation': 'Баг - это ошибка в программе, вызывающая неправильное или неожиданное поведение.'
        },
        {
            'question': 'Какие бывают виды тестирования по знанию системы?',
            'options': [
                'Черный ящик, белый ящик, серый ящик',
                'Быстрое и медленное',
                'Простое и сложное',
                'Ручное и автоматическое'
            ],
            'correct': 0,
            'explanation': 'Черный ящик (без знания кода), белый ящик (с доступом к коду), серый ящик (частичное знание).'
        },
        {
            'question': 'Что такое регрессионное тестирование?',
            'options': [
                'Тестирование после внесения изменений для проверки работы старого функционала',
                'Тестирование новых функций',
                'Тестирование производительности',
                'Тестирование безопасности'
            ],
            'correct': 0,
            'explanation': 'Регрессия проверяет, что новые изменения не сломали существующую функциональность.'
        },
        {
            'question': 'Что такое тест-кейс?',
            'options': [
                'Документ с набором шагов, входных данных и ожидаемых результатов',
                'Список найденных багов',
                'План тестирования',
                'Отчет о тестировании'
            ],
            'correct': 0,
            'explanation': 'Тест-кейс описывает, как именно протестировать конкретную функцию или сценарий.'
        },
        {
            'question': 'Что проверяется при функциональном тестировании?',
            'options': [
                'Соответствие функциональности требованиям',
                'Скорость работы приложения',
                'Безопасность системы',
                'Дизайн интерфейса'
            ],
            'correct': 0,
            'explanation': 'Функциональное тестирование проверяет, что система делает то, что должна делать.'
        },
        {
            'question': 'Что такое дымовое тестирование (smoke testing)?',
            'options': [
                'Быстрая проверка основного функционала после сборки',
                'Тестирование в дыму',
                'Тестирование производительности',
                'Финальное тестирование перед релизом'
            ],
            'correct': 0,
            'explanation': 'Smoke testing - беглая проверка критического функционала для решения о начале полного тестирования.'
        },
        {
            'question': 'Какой приоритет у критического бага?',
            'options': [
                'Highest/Critical - требует немедленного исправления',
                'Low - можно исправить потом',
                'Medium - исправить в следующем релизе',
                'Trivial - не требует исправления'
            ],
            'correct': 0,
            'explanation': 'Критические баги блокируют работу системы и требуют срочного исправления.'
        },
        {
            'question': 'Что такое верификация (verification)?',
            'options': [
                'Проверка соответствия продукта спецификации',
                'Проверка соответствия продукта ожиданиям пользователей',
                'Исправление багов',
                'Написание тестов'
            ],
            'correct': 0,
            'explanation': 'Верификация отвечает на вопрос: "Правильно ли мы создаем продукт?"'
        },
        {
            'question': 'Что такое валидация (validation)?',
            'options': [
                'Проверка соответствия продукта ожиданиям и потребностям пользователей',
                'Проверка соответствия продукта спецификации',
                'Установка программы',
                'Удаление программы'
            ],
            'correct': 0,
            'explanation': 'Валидация отвечает на вопрос: "Создаем ли мы правильный продукт?"'
        }
    ],
    'middle': [
        {
            'question': 'Что такое API тестирование?',
            'options': [
                'Тестирование программных интерфейсов приложения',
                'Тестирование пользовательского интерфейса',
                'Тестирование базы данных',
                'Тестирование сети'
            ],
            'correct': 0,
            'explanation': 'API тестирование проверяет взаимодействие между различными компонентами системы.'
        },
        {
            'question': 'Какие HTTP методы вы знаете?',
            'options': [
                'GET, POST, PUT, DELETE, PATCH',
                'SEND, RECEIVE, UPDATE',
                'READ, WRITE, EXECUTE',
                'OPEN, CLOSE, SAVE'
            ],
            'correct': 0,
            'explanation': 'GET (получить), POST (создать), PUT (обновить полностью), DELETE (удалить), PATCH (частичное обновление).'
        },
        {
            'question': 'Что такое SQL инъекция?',
            'options': [
                'Вид атаки, при котором вредоносный SQL код внедряется в запрос',
                'Метод оптимизации базы данных',
                'Способ резервного копирования',
                'Тип индекса в БД'
            ],
            'correct': 0,
            'explanation': 'SQL injection - критическая уязвимость, позволяющая выполнить произвольные SQL команды.'
        },
        {
            'question': 'Что проверяется при нагрузочном тестировании?',
            'options': [
                'Поведение системы под определенной нагрузкой',
                'Функциональность системы',
                'Безопасность системы',
                'Удобство интерфейса'
            ],
            'correct': 0,
            'explanation': 'Нагрузочное тестирование показывает, как система работает при большом количестве пользователей.'
        },
        {
            'question': 'Что такое CI/CD?',
            'options': [
                'Continuous Integration/Continuous Delivery - непрерывная интеграция и доставка',
                'Code Integration/Code Delivery',
                'Computer Integration/Computer Delivery',
                'Critical Integration/Critical Delivery'
            ],
            'correct': 0,
            'explanation': 'CI/CD - практика автоматической сборки, тестирования и развертывания кода.'
        },
        {
            'question': 'Какой код ответа HTTP означает успешный запрос?',
            'options': [
                '200 OK',
                '404 Not Found',
                '500 Internal Server Error',
                '403 Forbidden'
            ],
            'correct': 0,
            'explanation': 'Код 200 означает успешное выполнение запроса. 2xx - успех, 4xx - ошибка клиента, 5xx - ошибка сервера.'
        },
        {
            'question': 'Что такое mockito/mock объект?',
            'options': [
                'Имитация реального объекта для тестирования',
                'Реальный объект базы данных',
                'Файл конфигурации',
                'Тип переменной'
            ],
            'correct': 0,
            'explanation': 'Mock объекты имитируют поведение реальных объектов для изоляции тестируемого кода.'
        },
        {
            'question': 'Что означает принцип DRY в тестировании?',
            'options': [
                'Don\'t Repeat Yourself - не повторяйся',
                'Do Repeat Yourself - повторяй себя',
                'Delete Repeated Years',
                'Debug Ready Yearly'
            ],
            'correct': 0,
            'explanation': 'DRY - принцип избегания дублирования кода, используйте переиспользуемые функции и классы.'
        },
        {
            'question': 'Что такое Page Object Model (POM)?',
            'options': [
                'Паттерн проектирования для автоматизации, где каждая страница - это класс',
                'Модель базы данных',
                'Тип документации',
                'Метод ручного тестирования'
            ],
            'correct': 0,
            'explanation': 'POM помогает создать более поддерживаемые и читаемые автотесты через инкапсуляцию элементов страницы.'
        },
        {
            'question': 'Что проверяется при тестировании безопасности?',
            'options': [
                'Защищенность системы от несанкционированного доступа и атак',
                'Скорость работы системы',
                'Функциональность системы',
                'Внешний вид интерфейса'
            ],
            'correct': 0,
            'explanation': 'Security testing проверяет уязвимости системы: SQL injection, XSS, CSRF и другие атаки.'
        }
    ],
    'senior': [
        {
            'question': 'Что такое Test Driven Development (TDD)?',
            'options': [
                'Методология разработки, где тесты пишутся до кода',
                'Методология, где тесты пишутся после кода',
                'Инструмент для тестирования',
                'Язык программирования'
            ],
            'correct': 0,
            'explanation': 'TDD: Red (пишем тест) → Green (пишем код) → Refactor (улучшаем код).'
        },
        {
            'question': 'Что такое Behavior Driven Development (BDD)?',
            'options': [
                'Методология разработки, основанная на описании поведения системы',
                'Тип базы данных',
                'Инструмент автоматизации',
                'Протокол передачи данных'
            ],
            'correct': 0,
            'explanation': 'BDD использует естественный язык (Given-When-Then) для описания поведения системы.'
        },
        {
            'question': 'Какие метрики качества кода вы знаете?',
            'options': [
                'Code Coverage, Cyclomatic Complexity, Technical Debt',
                'Только количество строк кода',
                'Только количество функций',
                'Только количество классов'
            ],
            'correct': 0,
            'explanation': 'Метрики помогают объективно оценить качество кода и покрытие тестами.'
        },
        {
            'question': 'Что такое Contract Testing?',
            'options': [
                'Тестирование контрактов между микросервисами',
                'Тестирование юридических документов',
                'Тестирование UI',
                'Тестирование базы данных'
            ],
            'correct': 0,
            'explanation': 'Contract testing проверяет соблюдение договоренностей между сервисами (например, через Pact).'
        },
        {
            'question': 'Как называется тестирование, направленное на поиск уязвимостей через случайные данные?',
            'options': [
                'Fuzz Testing (фаззинг)',
                'Unit Testing',
                'Integration Testing',
                'Smoke Testing'
            ],
            'correct': 0,
            'explanation': 'Fuzzing отправляет случайные/некорректные данные для выявления неожиданного поведения и краш.'
        },
        {
            'question': 'Что такое Chaos Engineering?',
            'options': [
                'Практика внедрения отказов в систему для проверки отказоустойчивости',
                'Беспорядочное тестирование',
                'Тестирование без плана',
                'Тестирование хаотичного кода'
            ],
            'correct': 0,
            'explanation': 'Chaos Engineering (Netflix Chaos Monkey) - преднамеренное создание сбоев в продакшене.'
        },
        {
            'question': 'Что проверяют при стресс-тестировании?',
            'options': [
                'Поведение системы за пределами нормальной рабочей нагрузки',
                'Функциональность при нормальной нагрузке',
                'Удобство интерфейса',
                'Цвета дизайна'
            ],
            'correct': 0,
            'explanation': 'Stress testing проверяет точку отказа системы и поведение при критических нагрузках.'
        },
        {
            'question': 'Что такое Shift-Left Testing?',
            'options': [
                'Подход, при котором тестирование начинается на ранних этапах разработки',
                'Тестирование левой части интерфейса',
                'Тестирование левой кнопки мыши',
                'Перенос тестов влево в коде'
            ],
            'correct': 0,
            'explanation': 'Shift-Left - раннее вовлечение QA в процесс разработки для снижения стоимости багов.'
        },
        {
            'question': 'Какой паттерн используется для управления тестовыми данными?',
            'options': [
                'Test Data Builder, Object Mother, Fixture',
                'Только Singleton',
                'Только Factory',
                'Только Observer'
            ],
            'correct': 0,
            'explanation': 'Эти паттерны помогают создавать и управлять тестовыми данными более эффективно.'
        },
        {
            'question': 'Что такое A/B тестирование?',
            'options': [
                'Метод сравнения двух версий для определения лучшей',
                'Тестирование букв алфавита',
                'Тестирование первых двух функций',
                'Альтернативное название unit-тестирования'
            ],
            'correct': 0,
            'explanation': 'A/B тестирование показывает пользователям разные версии для определения более эффективной.'
        }
    ]
}

# Хранилище данных пользователей
user_data = {}

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Расширенное стартовое сообщение с выбором уровня"""
    keyboard = [
        [InlineKeyboardButton("Junior QA Engineer 🌱", callback_data='level_junior')],
        [InlineKeyboardButton("Middle QA Engineer 🚀", callback_data='level_middle')],
        [InlineKeyboardButton("Senior QA Engineer 👑", callback_data='level_senior')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    user_name = update.message.from_user.first_name or "друг"
    
    welcome_text = (
        f'👋 Привет, {user_name}!\n\n'
        f'Я — твой персональный помощник для подготовки к собеседованию на позицию QA Engineer.\n\n'
        f'🎯 <b>Что я умею:</b>\n'
        f'✅ Проверяю знания по 3 уровням сложности\n'
        f'✅ Задаю вопросы с вариантами ответов\n'
        f'✅ Даю подробные объяснения к каждому ответу\n'
        f'✅ Показываю детальную статистику результатов\n\n'
        f'📚 <b>Уровни тестирования:</b>\n'
        f'🌱 <b>Junior</b> — основы тестирования, базовая терминология\n'
        f'🚀 <b>Middle</b> — API, автоматизация, CI/CD, безопасность\n'
        f'👑 <b>Senior</b> — TDD/BDD, метрики, архитектурные подходы\n\n'
        f'💡 <b>Как это работает:</b>\n'
        f'1️⃣ Выбираешь свой уровень\n'
        f'2️⃣ Отвечаешь на 10 вопросов\n'
        f'3️⃣ Получаешь оценку и рекомендации\n'
        f'4️⃣ Можешь пройти тест заново или выбрать другой уровень\n\n'
        f'🎓 <b>Система оценки:</b>\n'
        f'• 90-100% — Отлично! 🌟\n'
        f'• 70-89% — Хорошо! 👍\n'
        f'• 50-69% — Удовлетворительно 📚\n'
        f'• 0-49% — Нужно больше практики 💪\n\n'
        f'⚡️ <b>Выбери свой уровень и начинай прокачивать свои навыки!</b>'
    )
    
    await update.message.reply_text(
        welcome_text, 
        reply_markup=reply_markup,
        parse_mode='HTML'
    )

async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработка нажатий на кнопки"""
    query = update.callback_query
    await query.answer()
    
    user_id = query.from_user.id
    
    # Выбор уровня
    if query.data.startswith('level_'):
        level = query.data.replace('level_', '')
        user_data[user_id] = {
            'level': level,
            'questions': random.sample(QUESTIONS[level], len(QUESTIONS[level])),
            'current_question': 0,
            'correct_answers': 0,
            'total_questions': len(QUESTIONS[level])
        }
        await send_question(query, user_id)
    
    # Ответ на вопрос
    elif query.data.startswith('answer_'):
        answer_idx = int(query.data.replace('answer_', ''))
        await check_answer(query, user_id, answer_idx)

async def send_question(query, user_id: int) -> None:
    """Отправка вопроса пользователю"""
    data = user_data[user_id]
    
    if data['current_question'] >= data['total_questions']:
        await show_results(query, user_id)
        return
    
    question_data = data['questions'][data['current_question']]
    question_num = data['current_question'] + 1
    
    # Создаем кнопки с вариантами ответов
    keyboard = []
    for idx, option in enumerate(question_data['options']):
        keyboard.append([InlineKeyboardButton(option, callback_data=f'answer_{idx}')])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    level_emoji = {'junior': '🌱', 'middle': '🚀', 'senior': '👑'}
    
    question_text = (
        f"{level_emoji[data['level']]} Вопрос {question_num}/{data['total_questions']}\n\n"
        f"❓ {question_data['question']}"
    )
    
    await query.edit_message_text(question_text, reply_markup=reply_markup)

async def check_answer(query, user_id: int, answer_idx: int) -> None:
    """Проверка ответа пользователя"""
    data = user_data[user_id]
    question_data = data['questions'][data['current_question']]
    
    is_correct = answer_idx == question_data['correct']
    
    if is_correct:
        data['correct_answers'] += 1
        message = (
            "✅ Правильно!\n\n"
            f"💡 {question_data.get('explanation', '')}"
        )
    else:
        correct_answer = question_data['options'][question_data['correct']]
        message = (
            f"❌ Неправильно!\n\n"
            f"Правильный ответ:\n{correct_answer}\n\n"
            f"💡 {question_data.get('explanation', '')}"
        )
    
    data['current_question'] += 1
    
    # Кнопка для следующего вопроса
    keyboard = [[InlineKeyboardButton("Следующий вопрос ➡️", callback_data='next_question')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(message, reply_markup=reply_markup)

async def next_question_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Переход к следующему вопросу"""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    await send_question(query, user_id)

async def show_results(query, user_id: int) -> None:
    """Показ результатов тестирования"""
    data = user_data[user_id]
    correct = data['correct_answers']
    total = data['total_questions']
    percentage = (correct / total) * 100
    
    # Определяем оценку
    if percentage >= 90:
        grade = "Отлично! 🌟"
        comment = "Ты отлично подготовлен к собеседованию!"
    elif percentage >= 70:
        grade = "Хорошо! 👍"
        comment = "Неплохой результат, но есть куда расти."
    elif percentage >= 50:
        grade = "Удовлетворительно 📚"
        comment = "Стоит подтянуть знания по некоторым темам."
    else:
        grade = "Нужно больше практики 💪"
        comment = "Рекомендую повторить материал и попробовать еще раз."
    
    level_names = {
        'junior': 'Junior QA Engineer 🌱',
        'middle': 'Middle QA Engineer 🚀',
        'senior': 'Senior QA Engineer 👑'
    }
    
    keyboard = [
        [InlineKeyboardButton("Пройти тест заново 🔄", callback_data=f"level_{data['level']}")],
        [InlineKeyboardButton("Выбрать другой уровень 🎯", callback_data='choose_level')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    results_text = (
        f"🎓 Результаты теста\n"
        f"{'='*25}\n\n"
        f"Уровень: {level_names[data['level']]}\n"
        f"Правильных ответов: {correct}/{total}\n"
        f"Процент: {percentage:.1f}%\n\n"
        f"{grade}\n"
        f"{comment}"
    )
    
    await query.edit_message_text(results_text, reply_markup=reply_markup)

async def choose_level_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Возврат к выбору уровня"""
    query = update.callback_query
    await query.answer()
    
    keyboard = [
        [InlineKeyboardButton("Junior QA Engineer 🌱", callback_data='level_junior')],
        [InlineKeyboardButton("Middle QA Engineer 🚀", callback_data='level_middle')],
        [InlineKeyboardButton("Senior QA Engineer 👑", callback_data='level_senior')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        '🎯 Выбери уровень для прохождения теста:',
        reply_markup=reply_markup
    )

def main() -> None:
    """Запуск бота"""
    # Получаем токен из переменной окружения
    token = os.getenv('BOT_TOKEN')
    
    if not token:
        logger.error("❌ Ошибка: Не указан токен бота!")
        logger.error("Укажите переменную окружения BOT_TOKEN")
        return
    
    logger.info("🚀 Инициализация бота...")
    application = Application.builder().token(token).build()

    # Регистрация обработчиков
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CallbackQueryHandler(button_callback, pattern='^(level_|answer_)'))
    application.add_handler(CallbackQueryHandler(next_question_callback, pattern='^next_question$'))
    application.add_handler(CallbackQueryHandler(choose_level_callback, pattern='^choose_level$'))

    # Запуск бота
    logger.info("🤖 Бот запущен и готов к работе!")
    application.run_polling(allowed_updates=Update.ALL_TYPES)

if __name__ == '__main__':
    main()
